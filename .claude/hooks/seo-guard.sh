...
case "$FILE" in
  ...
  */src/data/blogPosts.ts)
    MSG="Article de blog modifié. Publication d'un NOUVEL article : suivre le skill /blog-article. Ne pas oublier, en plus de blogPosts.ts : (1) l'URL dans public/sitemap.xml + le <lastmod> de /blog, (2) les TROIS fichiers LLM (llms.txt, llms-full.txt section 9 avec la ligne « Questions auxquelles il répond », chatbot-knowledge.txt avec les questions déclencheuses), (3) le pipeline image (job dans scripts/optimize-images.sh + image OG <slug>-og.jpg en 1200x630), (4) npm run build pour générer le HTML pré-rendu, (5) le post Google Business à rédiger pour Christophe (étape 9 du skill, modèles et prompt image dans docs/posts-google-blog.md). Vérifier l'absence de cadratin dans content/excerpt/metaDescription : grep sur blogPosts.ts, JAMAIS sur le HTML (le gabarit en compte 3, plus un par item de liste dessiné en CSS)."
    ;;
  */index.html)
  ...
  */src/App.tsx|*/src/index.css)
    MSG="Fichier porteur des garde-fous d'accessibilité. Vérifier qu'ils sont TOUJOURS là, Lighthouse ne les détecte pas : (1) le lien d'évitement <a href=\"#contenu\" class=\"skip-link\"> en tête de App.tsx, premier élément focusable, avec id=\"contenu\" sur le <main> ; (2) MotionConfig reducedMotion=\"user\" autour du <Router> ; (3) dans index.css, la classe .skip-link (invisible hors focus, ce n'est PAS du CSS mort) et la media query prefers-reduced-motion. Tester : première tabulation sur la home, le lien doit apparaître. Détail : section Accessibilité de CLAUDE.md"
    ;;
